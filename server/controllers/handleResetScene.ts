...
import { errorHandler, getCredentials, getPendingMessages, getThemeEnvVars, getWorldDataObject } from "../utils/index.js";
...
export const handleResetScene = async (req: Request, res: Response) => {
  try {
    ...
    const anchorAssetIds = anchorAssets
    ...
    if (shouldHardReset) {
      ...
      await world.updateDataObject({ [`scenes.${sceneDropId}.anchorAssets`]: anchorAssetIds }, { lock: { lockId, releaseLock: true } });
