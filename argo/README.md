...
**Secrets:** no plaintext committed. `bboard0-sealedsecret.yaml` holds only ciphertext
(sealed strict-scope for namespace sdk-apps-dev); the controller unseals it into
`Secret bboard0-secrets`, consumed via `envFrom`. Non-secret env (incl. public
`INTERACTIVE_KEY`) lives in `bboard0-config`.

## Environment

| Env | Service | Namespace | Host | Health |
| --- | ------- | --------- | ---- | ------ |
| `dev` | `bboard0` | `sdk-apps-dev` | bboard0-dev-topia.topia-rtsdk.com | `/api/system/health` |
